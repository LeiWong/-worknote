...
def spider_url(id,start,end):
	...
	while True:
		...
		proxies = {
			...
			"https": "https://211.103.250.145:80",
			"https": "https://118.144.151.145:3128"
		}
		...
		if item:
			...
			j = 0
			for each in item:
				name = each.xpath("//td[1]/text()")[j].encode("utf-8")
				price = each.xpath("//td[2]/text()")[j].encode("utf-8")
				market = each.xpath("//td[3]/a/text()")[j].encode("utf-8")
				date = each.xpath("//td[4]/text()")[j].encode("utf-8")
				...
				with open("./%s%s-%s.txt" % (product,s,e), "a+") as f:
					f.write(data)
				if j < len(item):
					j += 1
			print date.strip()
