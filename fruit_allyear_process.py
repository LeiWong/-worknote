	...
	while True:
		...
		if item:
			...
			for each in item:
				...
				price = each.xpath("//td[2]/text()")[j].encode("utf-8").strip()
				data["price"] = int(price)
				data["market"] = each.xpath("//td[3]/a/text()")[j].encode("utf-8").strip()
				date = each.xpath("//td[4]/text()")[j].encode("utf-8").strip()
				date2 = date.split("-")
				data["date"] =json.dumps(datetime.datetime(int(date2[0]),int(date2[1]),int(date2[2])))
				date = each.xpath("//td[4]/text()")[j].encode("utf-8").strip()
...
def run():
	...
	for year in [2014,2015,2016]:
		...
		for i in range(1, 5):
			if i == 1:
				data1 = {}
				data1["start"] = year + "-01-01"
				data1["end"] = year + "-03-31"
				data1["id"] = id
				q.put(data1)
			elif i == 2:
				data2 = {}
				data2["start"] = year + "-03-31"
				data2["end"] = year + "-06-30"
				data2["id"] = id
				q.put(data2)
			elif i == 3:
				data3 = {}
				data3["id"] = id
				data3["start"] = year + "-06-30"
				data3["end"] = year + "-09-30"
				q.put(data3)

			elif i == 4:
				data4 = {}
				data4["start"] = year + "-09-30"
				data4["end"] = year + "-12-31"
				data4["id"] = id
				q.put(data4)
		p1 = Process(target=spider_url, args=(q,))
		p2 = Process(target=spider_url, args=(q,))
		p3 = Process(target=spider_url, args=(q,))
		p4 = Process(target=spider_url, args=(q,))
		p1.start()
		p2.start()
		p3.start()
		p4.start()
